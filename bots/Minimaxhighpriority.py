...
class MinimaxhighpriorityBot(Bot):
    ...
    def value(
        ...
        Args:
            ...
            engine (GamePlayEngine): The game engine
            leader_move (Optional[Move]): The leader's move
            maximizing (bool): Whether the current player is maximizing or minimizing

        Returns:
            tuple[float, Optional[Move]]: The score and the corresponding move
        """
        ...
        sorted_moves = sorted(valid_moves, key=lambda move: scorer.rank_to_points(move.card.rank), reverse=True)
