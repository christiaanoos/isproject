...
class RdeephighpriorityBot(Bot):
    ...
    """
    def __init__(self, depth: int = 5, rand: random.Random = 42, num_samples: int = 10, name: Optional[str] = None) -> None:
        super().__init__(Bot)
    ...
    def __evaluate(self, gamestate: GameState, engine: GamePlayEngine, leader_move: Optional[Move], my_move: Move) -> float:
        ...
        if leader_move:
            ...
            leader_bot = FirstFixedMoveThenBaseBot(leader_move, RandBot(rand=self.__rand))
            me = follower_bot = FirstFixedMoveThenBaseBot(my_move, RandBot(rand=self.__rand))
        else:
            # I am the leader bot
            me = leader_bot = FirstFixedMoveThenBaseBot(my_move, RandBot(rand=self.__rand))
...
class FirstFixedMoveThenBaseBot(Bot):
    def _init_(self, first_move: Move, base_bot: Bot = RandBot(random.Random(42))) -> None:
